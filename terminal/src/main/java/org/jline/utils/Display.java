...
public class Display {
    ...
    public void update(List<AttributedString> newLines, int targetCursorPos, boolean flush) {
        synchronized (terminal.writer()) {
            ...
            final int nlines = newLines.size();
            final int olines = oldLines.size();
            int numLines = Math.min(rows, Math.max(olines, nlines));
            while (lineIndex < numLines) {
                AttributedString oldLine = lineIndex < olines ? oldLines.get(lineIndex) : AttributedString.NEWLINE;
                AttributedString newLine = lineIndex < nlines ? newLines.get(lineIndex) : AttributedString.NEWLINE;
                ...
                if (wrapNeeded && lineIndex == (cursorPos + 1) / columns1 && lineIndex < nlines) {
                ...
                for (int i = 0; i < diffs.size(); i++) {
                    final DiffHelper.Diff diff = diffs.get(i);
                    final int width = diff.text.columnLength();
                    final DiffHelper.Operation nextOP = i <= diffs.size() - 2 ? diffs.get(i + 1).operation : null;
                    final int nextWidth = nextOP != null ? diffs.get(i + 1).text.columnLength() : 0;
                    switch (diff.operation) {
                        case EQUAL:
                            ...
                            break;
                        case INSERT:
                            if (nextOP == DiffHelper.Operation.EQUAL) {
                                cursorPos = moveVisualCursorTo(currentPos);
                            ...
                            } else if (nextOP == DiffHelper.Operation.DELETE && width == nextWidth) {
                        ...
                        case DELETE:
                            ...
                            if (nextOP == DiffHelper.Operation.EQUAL && currentPos + nextWidth < columns) {
                                moveVisualCursorTo(currentPos);
                ...
                lineIndex++;
                boolean newWrap = !newNL && !noWrap && lineIndex < newLines.size();
                ...
                wrapNeeded = false;
                if (this.delayedWrapAtEol && !noWrap) {
            ...
            if(noWrap && oldPos == 0 && rows > nlines) {
